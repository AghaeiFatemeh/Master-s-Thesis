...
#########################################################################
#########################################################################
######################   ARNT Expression vs CIS    ######################
#########################################################################
#########################################################################


cleandfclinical <- cleandfclinical[!is.na(concomitant_in_situ)]
# Remove empty strings and make it a proper factor
cleandfclinical <- cleandfclinical[concomitant_in_situ %in% c("Yes", "No")]
cleandfclinical$concomitant_in_situ <- factor(cleandfclinical$concomitant_in_situ)

# Boxplot: ARNT Expression vs concomitant_in_situ
p_expr <- ggplot(cleandfclinical, aes(x = concomitant_in_situ, y = ARNT_expression, fill = concomitant_in_situ)) +
  geom_boxplot(outlier.shape = NA, alpha = 0.5) +
  theme_minimal() +
  labs(title = "ARNT Expression by concomitant_in_situ",
       x = "concomitant_in_situ", y = "ARNT Expression") +
  scale_fill_manual(values = c("Yes" = "#9ecae1", "No" = "#fcae91")) +
  theme(plot.title = element_text(hjust = 0.5))

# t-test (for 2 groups)
test_expr <- t.test(ARNT_expression ~ concomitant_in_situ, data = cleandfclinical)
...
ggsave(paste0(Address.OutputGraphs, "Boxplot_ARNTexpression_vs_concomitant_in_situ_ttest.png"),
       plot = p_expr, width = 6, height = 5)



# 1. Filter and prepare data
df_model <- cleandfclinical[!is.na(ARNT_expression) & !is.na(concomitant_in_situ)]
df_model$concomitant_in_situ <- factor(df_model$concomitant_in_situ)
df_model$concomitant_in_situ <- relevel(df_model$concomitant_in_situ, ref = "No")

# 2. Run logistic regression
model <- glm(concomitant_in_situ ~ ARNT_expression + Gender + age_at_recruit_year, data = df_model, family = binomial)
...
latex_lines <- c(
  ...
  "\\hline",
  "\\end{tabular}",
  "\\caption{Logistic regression results for ARNT expression predicting concomitant_in_situ.}",
  "\\label{tab:logistic_arnt_cis}",
  "\\end{table}"
)

# 8. Save as .tex file
writeLines(latex_lines, con = paste0(Address.OutputTables, "LogisticRegression_ARNT_concomitant_in_situ.tex"))
