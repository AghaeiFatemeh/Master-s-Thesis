...
dfSampleInfo <- dfSampleInfo[!is.na(Smoking_Status) & Smoking_Status != ""]


# Remove rows with NA Smoking_Status
dfSampleInfo <- dfSampleInfo[!is.na(Smoking_Status)]

# 1. Define smoking‐status levels, labels and colours
smoke_levels <- c("Smoker", "Ex-smoker", "Non-smoker")
smoke_labels <- c("Smoker", "Former smoker", "Never smoked")
smoke_cols   <- setNames(
...
ggsave(
  filename = paste0(Address.OutputGraphs, 
                    "Boxplot_ARNTexpression_vs_SmokingStatus_dfSampleInfo_Kruskal.png"),
  plot   = p_expr,
  width  = 6,
  height = 5
)
