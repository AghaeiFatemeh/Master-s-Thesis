...
cleandfclinical$Recurrence <- factor(cleandfclinical$Recurrence)

#########################################################################
#########################################################################
###################   ARNT Expression vs Recurrence    ##################
#########################################################################
#########################################################################


# Boxplot: ARNT Expression vs Recurrence
p_expr <- ggplot(cleandfclinical, aes(x = Recurrence, y = ARNT_expression, fill = Recurrence)) +
  geom_boxplot(outlier.shape = NA, alpha = 0.5) +
  theme_minimal() +
  labs(title = "ARNT Expression by Recurrence",
       x = "Recurrence", y = "ARNT Expression") +
  scale_fill_manual(values = c("TRUE" = "#9ecae1", "False" = "#fcae91")) +
  theme(plot.title = element_text(hjust = 0.5))

# t-test (for 2 groups)
test_expr <- t.test(ARNT_expression ~ Recurrence, data = cleandfclinical)
...
ggsave(paste0(Address.OutputGraphs, "Boxplot_ARNTexpression_vs_Recurrence_ttest.png"),
       plot = p_expr, width = 6, height = 5)



# 1. Filter and prepare data
df_model <- cleandfclinical[!is.na(ARNT_expression) & !is.na(Recurrence)]
df_model$Recurrence <- factor(df_model$Recurrence)
df_model$Recurrence <- relevel(df_model$Recurrence, ref = "False")

# 2. Run logistic regression
model <- glm(Recurrence ~ ARNT_expression + Gender + age_at_recruit_year, data = df_model, family = binomial)
...
latex_lines <- c(
  ...
  "\\hline",
  "\\end{tabular}",
  "\\caption{Logistic regression results for ARNT expression predicting Recurrence.}",
  "\\label{tab:logistic_arnt_cis}",
  "\\end{table}"
)

# 8. Save as .tex file
writeLines(latex_lines, con = paste0(Address.OutputTables, "LogisticRegression_ARNT_Recurrence_Table.tex"))
